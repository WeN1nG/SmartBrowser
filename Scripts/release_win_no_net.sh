#!/usr/bin/env bash
# 打包脚本：Windows 版 BrowserDemo 应用（不含 .NET 运行时，依赖目标机器已安装 .NET 8）
# 输出目录：C:\CodeSpace\Objects\Browser\Publish/Release/Win-FrameworkDependent
...
OUTPUT_BASE="$(cd "$(dirname "$0")/.." && pwd)/Publish/Release/Win-FrameworkDependent"
...
echo "===== 1. 发布应用 (SelfContained=false, Win-x64, ReadyToRun) ====="
...
# 2) 原生 DLL 保留在根目录
#    包括：WebView2Loader、WPF 原生桥接库、VC++ 运行时等
NATIVE_PATTERNS=(
  "WebView2Loader.dll"
  "*_cor3.dll"
...
================================================================
  SmartAI Browser Demo - 依赖 .NET 8 版本
================================================================
...
系统要求:
  - Windows 10/11 (x64)
  - 必须安装 .NET 8 Desktop Runtime (desktop-runtime)
  - WebView2 Runtime（Windows 10/11 通常已内置）

  .NET 8 Desktop Runtime 安装:
...
echo ""
echo "目标机器需要预先安装 .NET 8 Desktop Runtime 才能运行。"
