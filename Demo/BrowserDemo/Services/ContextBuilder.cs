...
public class ContextBuilder
{
    ...
    public string BuildSystemPrompt()
    {
        ...
        Logger.Debug($"ContextBuilder: 系统提示词已构建 ({result.Length} 字符)");
        Logger.Debug($"ContextBuilder: 动态上下文 — page=\"{CurrentPageTitle}\", url=\"{CurrentPageUrl}\"");
        return result;
    }
    ...
    public bool UnregisterTool(string name)
    {
        ...
        if (removed > 0)
            Logger.Debug($"ContextBuilder: 已移除工具 \"{name}\"");
        return removed > 0;
    }
    ...
    /// </summary>
    public void ImportSkillsFromRegistry(SkillSystemIntegration skillSystem)
    {
        using var _ = Logger.Trace("ContextBuilder::ImportSkillsFromRegistry");
        if (skillSystem == null || !skillSystem.IsInitialized) return;

        var importedCount = 0;

        // ★★★ 先注册 MCP 原子工具（直接暴露给 AI）★★★
        foreach (var mcpTool in skillSystem.McpClient.Tools)
        {
            var existing = RegisteredTools.FindIndex(t => t.Name == mcpTool.Name);
            var toolDef = new ToolDefinition
            {
                Name = mcpTool.Name,
                Description = $"[MCP] {mcpTool.Description}",
                Parameters = McpSchemaToParams(mcpTool.InputSchema)
            };

            if (existing >= 0)
                RegisteredTools[existing] = toolDef;
            else
                RegisteredTools.Add(toolDef);

            importedCount++;
        }

        // ★★★ 注册组合技能（多步工作流）★★★
        foreach (var composite in skillSystem.Registry.CompositeSkills)
        {
        ...
        Logger.Info($"ContextBuilder: 从 MCP 技能系统导入了 {importedCount} 个工具 ({skillSystem.McpClient.Tools.Count} MCP + {skillSystem.Registry.CompositeSkills.Count()} 组合)");
    }
    ...
    private void AppendIdentity(StringBuilder sb)
    {
        sb.AppendLine("# 身份设定");
        sb.AppendLine();
    ...
    private void AppendOutputFormat(StringBuilder sb)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("### 结构化思考字段（每次回复必须包含）");
        sb.AppendLine("每次工具调用后，必须在回复中包含以下三个字段：");
        sb.AppendLine();
        sb.AppendLine("```");
        ...
        sb.AppendLine();
        sb.AppendLine("### 完整回复格式");
        sb.AppendLine("1. **[思考过程]**：任务执行中的主输出。包括：");
        sb.AppendLine("   - 结构化思考字段（[上步评估]、[进度记忆]、[下步目标]）");
        sb.AppendLine("   - 对页面内容的必要分析和判断");
        sb.AppendLine("2. **[结论]**：只在任务完成、失败、无法继续或给最终答复时输出。");
        sb.AppendLine();
        sb.AppendLine("执行中示例：");
        sb.AppendLine("```");
        sb.AppendLine("[思考过程]");
        sb.AppendLine("[上步评估]");
        sb.AppendLine("  - 操作：browser_navigate to https://example.com");
        sb.AppendLine("  - 结果：成功，HTTP 200");
        sb.AppendLine("  - 问题：无");
        sb.AppendLine("[进度记忆]");
        sb.AppendLine("  - 已完成：登录");
        sb.AppendLine("  - 进行中：查找目标页面");
        sb.AppendLine("  - 待完成：完成表单提交");
        sb.AppendLine("[下步目标]");
        sb.AppendLine("  - 目标：调用 observe_browser 观察页面状态");
        sb.AppendLine("  - 依据：导航成功后需要锚定当前页面内容");
        sb.AppendLine("```");
        sb.AppendLine();
        sb.AppendLine("重要规则：");
    ...
    /// <summary>将 MCP InputSchema 转换为 AI 可理解的 ToolDefinition 参数字典</summary>
    private static Dictionary<string, object?> McpSchemaToParams(System.Text.Json.JsonElement? schema)
    {
            ...
            if (schema.Value.TryGetProperty("properties", out var props))
            {
                foreach (var prop in props.EnumerateObject())
                {
                    ...
                    if (prop.Value.TryGetProperty("type", out var typeEl))
                        propDef["type"] = typeEl.GetString() ?? "string";

                    if (prop.Value.TryGetProperty("description", out var descEl))
                        propDef["description"] = descEl.GetString() ?? prop.Name;

                    result[prop.Name] = propDef;
                }
            }
        }
        catch { }
    ...
    private static ToolDefinition CompositeToToolDefinition(Skills.CompositeSkillDefinition composite)
    {
